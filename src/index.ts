...
export default class LinkedList<T> {
    ...
    public remove(index: number) {
        ...
        if (index == 0 && this.head?.next instanceof Node) {
            this.head = this.head.next;
            return true;
        }
        let prev: Node<T> | null | undefined = this.head;
        let current = 1;
        let node: Node<T> | null | undefined = this.head?.next;
        while (node) {
            if (current == index) {
                prev.next = node.next;
                this.length--;
     ...
     * Sorting
     */
    public sort(order: number = 1) {
        let s = new Sort<T>();
        // Apply merge Sort
        this.head = s.mergeSort(this.head, order);
    }
